...
> 当前提交：`122f4c07`
> 状态：核心布局和测试基础设施已建立；测试场布局验收通过，真实业务流程仍按窗口逐项推进。
...
- HUD 玩家球已完成单控件切换：移除误复用 `GameInter[62]` 造成的右侧重复红球；无最大魔法值显示完整红球，有最大魔法值在同一 `(49,13)`、`112×110` 控件显示红蓝球，并加入状态回归审计。
...
- 修正人物旧版装备槽 enum 映射：`Helmet(2)` 为左下 `(27,264)`，`Torch(3)` 为顶部 `(177,70)`；审计现在逐项检查 8 个槽位的可见性、38×38 尺寸和坐标。提交 `b64c0b37`。
- 测试场新增窗口生命周期审计：15 个窗口逐个验证打开、重复打开不重复入栈、关闭最上层和窗口栈清空；`LegacyAudit` 输出 `lifecycle=True`。
...
尚未完成：旧版属性文本、纸娃娃和装备数据在两个旧版页面中的真实角色绑定，以及真实换装/拖动/快捷键验收；空数据测试场只验收布局和控件结构。
...
- **此节的早期实施描述已被全量审计撤回，不可作为 EI 布局规格或验收依据。**审计见 [LEGACY_EI_UI_AUDIT_2026-09-23.md](LEGACY_EI_UI_AUDIT_2026-09-23.md) 的 SKL-01..10：EI id14主初始化记录根尺寸296×332；当前 Godot 452×380只是测试profile，目标F400裁剪/配准未闭合；原版primary-static证明六个列表命中RECT，而当前12格布局只属实现差异；旧Client或当前MagicInfo图标不能替代EI `[skill+6]` 到 MIcon帧的映射证据。

尚未完成：根RECT/裁剪、列表与分类映射、左右页及箭头状态、逐技能图标/详情映射、EI F1-F12输入链及目标版运行视觉验收。该摘要里记载的Godot行为仅作为当前实现说明，不能视为EI等价证据。
