...
物品业务扩展审计（`--operation-audit-ext`）真实运行结果：S1 药水使用/解锁、S2/S3 锁定解锁、S4～S8 戒指/手镯卸下与替换、S9 腰带绑定、S10 自动药水绑定、S11 清理、S12 装备恢复均通过。S13 邮件附件发送因服务端返回 `success=false` 停止，不能计为通过；基础 `--operation-audit` 另因测试角色当时没有可用的已装备槽而在装备阶段停止。上述失败已保留为后续业务验收项。
