...
- **本轮补做的技能 ID 映射检查**：原版右页用 `Magic.exp` 的 `#N`（例如 `#3 基本剑术`）索引，而当前 `MagicInfo` 至少有三种不同标识：DB 顺序 `Index`、业务 `MagicType`、`Name`。交叉核对 `magic-exp-records.json`、`MagicInfo.cs`、174 条现代 `MagicInfo` 数据以及 `translations/db_names.json` 后，中文名能直接匹配的旧版记录目前只有 22/50；例如旧版 `#3 基本剑术` 对应现代 `Swordsmanship`（当前 DB Index=1），`#7 攻杀剑术` 对应 `Slaying`（Index=3），`#12 刺杀剑术` 对应 `Thrusting`（Index=4），`#25 半月弯刀` 对应 `Half Moon`（Index=5）。这明确否定了用 `MagicInfo.Index` 直接代替 EI 技能 ID，也说明仅凭当前本地化词典无法恢复全部 50 条一一映射；剩余名称有版本差异、未翻译和技能集差异，不能静默按列表顺序补齐。研究资料中 Mud3 服务端 `magic.dat` 为另一套 105 条记录，只有 9 条抽样记录做了客户端 `Magic.exp` ID 的名字交叉核对，且工件明确警告不可无比较键直接合并；客户端技能记录是由服务端 packet-fed。因此 `magic.dat` 只能为已核对名称提供旁证，不能当作缺失 EI 客户端资源的直接替代。后续应继续找 EI 目标版本的技能网络记录/客户端构造链，并用名字、职业/元素和逐级要求交叉验证；在映射闭合前，右页正文仍不能安全地以现代 MagicInfo 代填。
