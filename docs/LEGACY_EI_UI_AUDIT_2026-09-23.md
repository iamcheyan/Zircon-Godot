...
**背包图标资源链复核：**`inventory-window-render-evidence.json` 记录原版物品 frame WORD 来自物品数据 `+0x28`，用于 `0x42F6D0` 的物品尺寸查询及 `0x466130` 图标绘制；该绘制上下文的 selector 是 `0x5668C4`，WIL 句柄表将 el82 绑定为 `Inventory.wil`。当前 `DXItemCell.ItemIconLibraryFile` 却固定为 `LibraryFile.StoreItem`，`DrawItemIcon()` 取现代 `ItemInfo.Image` 并经 `MirSkin.GetTexture()` 从常规 `Data/StoreItem.Zl` 查帧。本机 EI `inventory.wil/.wix` 独立读取到 1,440 个 WIL 索引；常规 `StoreItem.Zl` 含 8,590 个元数据索引。常规 `Data/Inventory.Zl` 有8,153个元数据索引；与 EI `inventory.wil` 按同一帧号核对宽、高及offset时，仅445/1,440帧头一致，剩余包含空帧错位或尺寸差异。用独立 `wilsdk.py` 与 `zlsdk.py` 解码这445个同元数据帧后，RGBA 字节没有一帧逐像素完全相同；RGB565→ZL有损编码本身可造成微差，因此不能把“非逐字节相等”一概解释为图案错误。并排放大检查F0/F1/F4时两边图案相近，F8则是明显不同的图案，抽样图见 `/tmp/ei-inventory-storeitem-sample.png`；同号不能普遍当成视觉等价。该F8只证明图库同号存在内容差异，不证明当前角色 `Info.Image` 实际引用的正是F8。普通 `DXItemCell` 也没有选用常规 `Inventory.Zl`，而固定走 `StoreItem.Zl`；EI目录没有`Inventory.Zl`，Godot没有通用WIL运行时读取器，故现有legacy背包代码不能证明正在使用EI el82的目标帧。这不只是槽位摆放差异，还意味着EI frame ID→当前 `Info.Image` 映射、完整画布/透明偏移及像素等价性均未闭合；需逐物品核对服务器 item data `+0x28` 与当前 `Info.Image`，并建立可复现的EI WIL解码/绘制路径后再验像素和多格占位。
