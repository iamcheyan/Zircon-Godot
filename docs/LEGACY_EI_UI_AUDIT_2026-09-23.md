...
**背包图标资源链复核：**`inventory-window-render-evidence.json` 记录原版物品 frame WORD 来自物品数据 `+0x28`，用于 `0x42F6D0` 的物品尺寸查询及 `0x466130` 图标绘制；该绘制上下文的 selector 是 `0x5668C4`，WIL 句柄表将 el82 绑定为 `Inventory.wil`。当前 `DXItemCell.ItemIconLibraryFile` 却固定为 `LibraryFile.StoreItem`，`DrawItemIcon()` 取现代 `ItemInfo.Image` 并经 `MirSkin.GetTexture()` 从常规 `Data/StoreItem.Zl` 查帧。本机 EI `inventory.wil/.wix` 独立读取到 1,440 个 WIL 索引；常规 `StoreItem.Zl` 含 8,590 个元数据索引，前几个索引在两边有相同画布尺寸/offset（如 F0 16×26、offset(-24,-16)），但格式与总索引范围不同，且本轮没有完成全帧 RGBA 像素比对，不能据此认定相同或不同像素。本机常规 `Data/Inventory.Zl` 有8,153个元数据索引；与 EI `inventory.wil` 按同一帧号核对宽、高及offset时，仅445/1,440帧头一致，剩余包含空帧错位或尺寸差异。这提示常规 `Inventory.Zl` 可能保留部分 EI 帧布局，但不足以证明像素相等；且普通 `DXItemCell` 当前没有选用这个库。本机 EI 目录没有 `Inventory.Zl`；Godot 没有通用 WIL 运行时读取器，故现有 legacy 背包代码不能证明其正在用 EI el82 的目标帧。这不只是槽位摆放差异，还意味着 EI frame ID→当前 `Info.Image` 映射、完整画布/透明偏移及像素相等性均未闭合；需逐物品核对服务器 item data `+0x28` 与当前 `Info.Image`，并建立可复现的 EI WIL 解码/绘制路径后再验像素和多格占位。
