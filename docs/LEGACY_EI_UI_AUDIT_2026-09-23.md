...
展开开关的静态预期与屏幕上的实际运动需要分开。此前已提交版的 `ToggleLegacyView()` 不写根 `Position`，把根窗宽从244改为520，把F200/F201独立背景分别放到`(-6,-92)`/`(-252,-92)`；但当前工作区有未提交改动，改为单背景按态切换F200/F201、隐藏/显示两组文字，必须以该版本重新做屏幕追踪。viewer API PNG alpha bbox 为F200 `(6,92,241,327)`、F201 `(252,92,518,327)`，新旧代码都能从静态字段推算左上像素，不能单凭此证明切换时无抖动；F201 画布本身的素材、根裁剪和交互区域需要结合更新后截图重新核。`GameScene.CreateHud()`初始化时根窗位置为`(0,0)`，`LayoutHud()`未见尺寸改变时重定位，但点击态帧/事件时序、窗口父节点变换、viewport/UI scale 的取整及实际裁切尚未观测。后续运行对照应分别跟踪根窗屏幕左上角、背景有效像素左上角、切换按钮hit rect、展开面板右边缘和同一背景像素，逐帧记录按下/释放/重绘，不能只比较整个窗口外接矩形。该 PNG bbox 是资源视觉证据，切换像素行为当前不标runtime-verified。
